...
Actual projected stem area index Sp (SAI), is assumed proportional to HEIGHT following [Federer et al. (1996)](./doc_literature.html), so
...
###Function ESAT
The subroutine ESAT calculates the saturated vapor pressure (kPa), e*, from temperature (°C), T, and calculates $\Delta$. The equations of [Murray (1967)](./doc_literature.html) are used.
...
When there is SNOW, RSS is set to zero, which affects calculated transpiration and interception rates, but there is no soil evaporation and snow evaporation is calculated using subroutine [SNOVAP](./doc_sno.html). This leaves something to be desired, but the whole situation with melting and evaporating snow as well as transpiration is a complicated mess.
...
Intrinsic functions for ROUGH are LOG and EXP. Z0C is calculated depending on HEIGHT. Z0 and DISP are depending on RATIO. If Ratio is >= 1 then it is calculated for closed canopy else for sparse canopy modified from [Shuttleworth and Gurney (1990)](./doc_literature.html). The resulting output of ROUGH is:
...
###Function SRSC
This routine obtains the canopy surface resistance, rsc, which is the classic canopy resistance in the Penman-Monteith equation, using the [Jarvis (1976)](./doc_literature.html) expression for the factors that control the individual leaf resistance, r, and its reciprocal the leaf conductance, g. 
...
[Stewart (1988)](./doc_literature.html) and [Saugier and Katerji (1991)](./doc_literature.html)) and then the vapor deficit limitation is done ([Lohammar et al. (1980)](./doc_literature.html)). A temperature limitation follows and then the canopy surface resistance RSC (s/m) is the output of SRSC calculated as a reciprocal of GSC.
...
* RS
* RA - as in [Shuttleworth and Wallace (1985)](./doc_literature.html)
...
###Function SWGRA
The three SW aerodynamic resistances, raa, ras, and rac are obtained in subroutine SWGRA by the methods of [Shuttleworth and Gurney (1990)](./doc_literature.html). 
...
* PMS
* PMC
* D0 - vapor pressure deficit, as in [Shuttleworth and Wallace (1985)](./doc_literature.html)
* CCS
* CCC - as CC and CS in [Shuttleworth and Wallace (1985)](./doc_literature.html)
* LE - total latent heat flux density (W/m2)
...
If daily solar radiation (SOLRAD) is input as zero, it is estimated as 0.55 * I0HDAY, or 55% of the potential solar radiation for the doy and location. The 0.55 value is an overall generalization for the United States, where values range from 0.50 in the east to 0.60 in the west ([U.S. Department of Commerce (1968)](./doc_literature.html)).
...
If daily average wind speed at a weather station (UW) is input as zero, it is estimated as 3 m s-1. This is a surprisingly good approximation for most weather stations in the United States at all seasons of the year ([U.S. Departement of Commerce (1968)](./doc_literature.html)). For other default values, enter the value as UA for each day in the data file. Note: measured values of zero wind speed should be entered as 0.1. 
